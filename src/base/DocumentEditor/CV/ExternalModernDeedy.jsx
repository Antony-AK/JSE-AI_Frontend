    ...
    <div className="flex flex-col gap-2 px-6 py-3 w-full h-full">
            ...
            {education.content.map(
              (entry, idx) =>
                entry.degree?.trim() && (
                  <p
                    key={idx}
                    className="text-sm ml-2 text-gray-700 mt-2"
                    style={{ breakInside: "avoid" }}
                  >
                    {entry.degree}
                  </p>
            ...
            >
              ...
              {Array.isArray(proj.Description) &&
                proj.Description.length > 0 && (
                  <ul className="list-none ml-3 mt-2 text-[13px] text-gray-800 space-y-1 leading-5">
      ...
      {skills.content.filter((skill) => skill.trim() !== "").length > 0 && (
        <div className="flex flex-col gap-1 avoid-page-break mt-5">
          ...
          <div
            className="text-[13px] ml-2 mt-2 text-gray-800"
            style={{ breakInside: "avoid" }}
          >
            {skills.content
